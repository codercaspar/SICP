...
(define (ripple-carry-adder a b c-in s c-out)
	(let ((c (make-wire)))
		(cond ((null? a) (set-signal! c-out (get-signal c-in)))
			  (else 
				(full-adder (car a) (car b) c-in (car s) c)
				(ripple-carry-adder (cdr a) (cdr b) c (cdr s) c-out)))))
...
(ripple-carry-adder a b c-in s carry)

(for-each (lambda (x) (set-signal! x 1)) a)
(for-each (lambda (x) (set-signal! x 1)) b)
