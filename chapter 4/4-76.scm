...
(define (merge-two-frame f1 f2)
    (if (null? f1)
	    f2
		(let ((v (car f1)))
		    (let ((var   (binding-variable v))
			      (value (binding-value v)))
				(let ((extension (extend-if-possible var value f2)))
				    (if (equal? extension 'failed)
				        'failed
					    (merge-two-frame (cdr f1) extension)))))))
...
(define (uniquely-asserted operands frame-stream)
    (stream-flatmap
        (lambda (frame)
            (let ((eval-stream (qeval (query-of-unique operands) (singleton-stream frame))))
...
(define (unify-match p1 p2 frame)
    ...
    (cond ((eq? frame 'failed) 'failed)
          ...
          ((and (pair? p1) (pair? p2)) (unify-match (cdr p1) (cdr p2) (unify-match (car p1) (car p2) frame)))
...
(define (interleave-delayed s1 delayed-s2)
    ...
    (if (stream-null? s1)
        ...
        (cons-stream (stream-car s1) (interleave-delayed (force delayed-s2) (delay (stream-cdr s1))))))
...
(define (map-over-symbols proc exp)
    ...
    (cond ((pair? exp) (cons (map-over-symbols proc (car exp)) (map-over-symbols proc (cdr exp))))
...
(define (expand-question-mark symbol)
    ...
    (let ((chars (symbol->string symbol)))
        ...
        (list '? (string->symbol (substring chars 1 (string-length chars)))) symbol)))
