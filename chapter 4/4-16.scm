...
(define (scan-out-defines body)
	(define (analyse input-body)
		(let ((defines (filter (lambda (x) (and (eq? 'define (car x)) (symbol? (cadr x)))) input-body))
			  (others  (filter (lambda (x) (not (and (eq? 'define (car x)) (symbol? (cadr x))))) input-body)))
			(if (null? defines)
				input-body
				(let ((let-syntax (map (lambda (x) (list (definition-variable x) ''*unassigned*)) defines))
				      (set-syntax (map (lambda (x) (make-set! (definition-variable x) (definition-value x))) defines)))
...
(define (eval-unbound! exp env)
	(if (eq? env the-empty-environment)
		(error "Unbound variable -unbound!" exp)
		(let ((frame (first-frame env))
	  		  (var (cadr exp)))
		    (if (lookup-variable-value-in-environment var frame)
