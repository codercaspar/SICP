...
(define (let-named? exp)
	(and (not (null? (cadr exp))) (not (pair? (cadr exp)))))
...
(define (letrec-binds exp)
	(cadr exp))

(define (letrec-body exp)
	(cddr exp))

(define (letrec->inter-define exp)
	(make-let
		(list)
		(append 
			(map (lambda (x) (append (list 'define) x)) (letrec-binds exp)) 
			(letrec-body exp))))

(put 'eval 'letrec (lambda (exp env) (my-eval (letrec->inter-define exp) env)))
