...
(define (my-filter pred? input)
	(if (null? input)
		'()
		(if (pred? (car input))
			(append (list (car input)) (my-filter pred? (cdr input)))
			(my-filter pred? (cdr input)))))
...
(define (scan-out-defines body)
	(define (analyse input-body)
		(let ((defines (my-filter (lambda (x) (and (definition? x) (definition-variable? x))) input-body))
			  (others  (my-filter (lambda (x) (not (and (definition? x) (definition-variable? x)))) input-body)))
			(if (null? defines)
				input-body
				(let ((let-syntax (map (lambda (x) 
					  ...
					  (set-syntax (map (lambda (x) 
					  				   ...
					  				   (my-filter (lambda (x) (pair? (definition-value x))) defines))))
...
(define (text-of-quotation exp env)
	(let ((quote-operand (cadr exp)))
		(if (not (pair? quote-operand))
			quote-operand
			(my-eval (make-lazy-list quote-operand) env))))

(define (make-lazy-list eles)
	(if (null? eles)
		'()
		(list 'cons (car eles) (make-lazy-list (cdr eles)))))
