...
(define (install-polynomial-package)
	...
	(define (gcd-terms a b)
		(if (empty-termlist? b)
			(reduce-single-terms a)
			(gcd-terms b (pseudoremainder-terms a b))))
	(define (reduce-single-terms a)
		(let ((gcd-division (apply gcd (map coeff a))))
				(map (lambda (term) (list (order term) (div (coeff term) gcd-division))) a)))
	(define (mul-single-terms a factor)
		(map (lambda (term) (list (order term) (* factor (coeff term)))) a))
	(define (reduce-terms a b)
		(let ((gcd-division-terms (gcd-terms a b)))
			(let ((t1 (first-term a))
				  (t2 (first-term b))
				  (t3 (first-term gcd-division-terms)))
				(let ((factor (expt (coeff t3) (+ 1 (- (max (order t1) (order t2)) (order t3))))))
					(list (reduce-single-terms (quotient-terms (mul-single-terms a factor) gcd-division-terms))
						  (reduce-single-terms (quotient-terms (mul-single-terms b factor) gcd-division-terms)))))))
	...
	(define (pseudoremainder-terms a b)
		(let ((t1 (first-term a))
			...
			(let ((factor (expt (coeff t2) (+ 1 (- (order t1) (order t2))))))
					(remainder-terms (mul-single-terms a factor) b))))
...
(define (drop-value z)
	(cond ((equal? (type-tag z) 'scheme-number) z)
		  ...
		  ((and (equal? (type-tag z) 'rational)
				(or (equal? (type-tag ((get 'numer 'rational) (cadr z))) 'polynomial)
					(equal? (type-tag ((get 'denom 'rational) (cadr z))) 'polynomial)))
			z)
...
(define (install-scheme-number-package)
	...
	(put 'reduce '(scheme-number scheme-number)
		(lambda (n d) (let ((g (greatest-common-divisor n d)))
...
(define mp1 (make-polynomial 'x '((1 1) (0 1))))
(define mp2 (make-polynomial 'x '((3 1) (0 -1))))
(define mp3 (make-polynomial 'x '((1 1))))
(define mp4 (make-polynomial 'x '((2 1) (0 -1))))
(define rf1 (make-rational mp1 mp2))
(define rf2 (make-rational mp3 mp4))
rf1
rf2
(add rf1 rf2)