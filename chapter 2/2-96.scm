...
(define (install-polynomial-package)
	...
	(define (negation-terms L)
		(map (lambda (term) (list (order term) (negation-value (coeff term)))) L))
	(define (negation p)
	...
	(define (div-poly p1 p2)
		(if (same-variable? (variable p1) (variable p2))
			(let ((result (quotient-terms (term-list p1) (term-list p2))))
	...
	(define (gcd-terms a b)
		(if (empty-termlist? b)
			(let ((gcd-division (apply gcd (map coeff a))))
				(map (lambda (term) (list (order term) (div (coeff term) gcd-division))) a))
	...
	(define (pseudoremainder-terms a b)
		(let ((t1 (first-term a))
			...
			(let ((factor (expt (coeff t2) (+ 1 (- (order t1) (order t2))))))
					(remainder-terms (map (lambda (term) (list (order term) (* factor (coeff term)))) a)
									 b))))
					...
					(let ((new-c (div (coeff t1) (coeff t2)))
						...
						(let ((rest-of-result 
								(div-terms 
									(add-terms 
										L1 
										(negation-terms (mul-terms (list (make-term new-o new-c)) L2))) 
									L2)))
							(list
								(add-terms (list (make-term new-o new-c)) (car rest-of-result))
								(cadr rest-of-result))))))))
...
(div (make-polynomial 'x (list (list 5 1) (list 0 (- 0 1))))
	 (make-polynomial 'x (list (list 2 1) (list 0 (- 0 1)))))
...
(define pp1 (make-polynomial 'x '((4 1) (3 -1) (2 -2) (1 2))))
(define pp2 (make-polynomial 'x '((3 1) (1 -1))))
(greatest-common-divisor pp1 pp2)
(define rrf (make-rational pp1 pp2))
rrf
...
(define rf (make-rational q1 q2))
rf

(greatest-common-divisor (make-polynomial 'x '((3 1)(0 1))) (make-polynomial 'x '((2 1)(0 1))))