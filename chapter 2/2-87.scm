...
(define (install-polynomial-package)
	...
	(put 'equal-zero? '(polynomial) 
		(lambda (p) (if (empty-termlist? (term-list p))
						#t
						(apply equal-zero? (map coeff (term-list p))))))
	'done)
...
(equal-zero? (make-polynomial 'x (list (list (make-scheme-number 1) (make-scheme-number 0)))))
(equal-zero? (make-polynomial 'x (list (list (make-scheme-number 1) (make-polynomial 'y (list))))))
